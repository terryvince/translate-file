...
<template>
	<view class="shopManage">
		...
		<!-- <view class="bg-white comment-box line-down">
			<view class="flex-main-between">
				...
				<view>
					...
					<text class="fs-28 style-receive color-white">ဖောက်သည်ဝန်ဆောင်မှုကိုဆက်သွယ်ပါ</text>
				</view>
			</view>
		...
		<shop-intro v-if="shopInfo" :shop-info="shopInfo">
			<text @click="contactCustomer()" class="fs-28 style-receive color-white">ဖောက်သည်ဝန်ဆောင်မှုကိုဆက်သွယ်ပါ</text>
		</shop-intro>
		<view class="goodsCoupon" :class="couponList.length>0?'goodsCouponTop':''">
			...
			<view class="goodsList" :class="couponList.length>0?'goodsListTop':''">
				<view class="nav acea-row row-middle">
					<view class="item" :class="['item',title ? 'font-color-red' : '']" @click="set_where(0)">{{ title ? title : "ပြည့်စုံသော" }}</view>
					...
					<view class="item" :class="nows ? 'font-color-red' : ''" @click="set_where(3)">ထုတ်ကုန်အသစ်</view>
				</view>
...
<script>
	...
	export default {
		...
		data: function() {
			return {
				...
				where: {
					...
					limit: 10,
					// keyword: s,
					// sid: id, //အလယ်တန်းခွဲခြားid
					news: 0,
					priceOrder: "",
					salesOrder: "",
					// type: 0, // 0 အထွေထွေကုန်ပစ္စည်း 1ကုန်ပစ္စည်းအမှတ် 2Featured ထုတ်ကုန်များ
					merId: 0,
				},
				// နောက်ဆုံးနှိပ်ထားသော filter ကိုမှတ်တမ်းတင်ပါ
				clickChange: 0,
				clickChangePrev: 0,
				// ကူပွန်
				couponList: []
		...
		methods: {
			...
			// စတိုးဆိုင်အချက်အလက်များကိုရယူပါ
			...
			get_product_list() {
				...
				getProducts(this.where).then(res => {
					...
					console.error("ထုတ်ကုန်စာရင်းရရှိရန်ချို့ယွင်းချက်！", err)
				})
			},
			//တည်ဆောက်သည်whereအခွအေနေ
			setWhere: function() {
				let that = this;
				if (that.price === 0) {
			...
			//ဖြစ်ရပ်ကိုင်တွယ်ခြင်းကို filter လုပ်ရန်ကိုနှိပ်ပါ
