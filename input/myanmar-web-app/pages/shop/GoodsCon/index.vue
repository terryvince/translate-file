...
<template>
	<view :class="productConClass">
		<view v-if="storeInfo.id || storeInfo.id===0">
			...
			<!-- စျေးနှုန်းခွဲဝေမှုဘား +level||0 vipတန်းမရှိပါ，ပထမဆုံးဖျောက်ထားပါ-->
				...
				<view class="meta-wrap flex-1">
					...
					<view class="flex-main-between">
						<view class="flex-main-start flex-baseline">
							<text class="left-20 fs-28 color-type style-type" style="color: #6E77FF;" v-if="storeInfo.type===1">အရေးပါသော</text>
							<text class="left-20 fs-28 color-type style-type" v-if="storeInfo.type==2">ကဗျာ</text>
							<text v-if="storeInfo.vipPrice && storeInfo.vipPrice > 0" class="left-20" style="font-size: 38rpx;">{{ mode=='point' ? `${storeInfo.vipPrice}အရေးပါသော` : `&yen;${storeInfo.vipPrice}` }}</text>
							<text class="fs-30 left-20 del-price-line" v-if="storeInfo.type!=1">မူလစျေးနှုန်း{{ `&yen;${storeInfo.otPrice}` }}</text>
						</view>
			...
			<view class="wrapper over-hide">
				...
				<view class="share flex-main-between line-top">
					<view>
						<text class="fs-24 color-number">သင်္ဘောဖြင့်ကုန်ပစ္စည်းပို့ခြင်း：{{ tempName }}</text>
					</view>
					<view>
						<text class="fs-24 color-number">ကုန်ပစ္စည်းလက်ဝယ်ရှိ{{ storeInfo.stock }}{{ storeInfo.unitName }}</text>
						<text class="fs-24 color-number left-30">ရောင်းချခဲ့သည်{{ storeInfo.sales }}{{ storeInfo.unitName }}</text>
					</view>
				</view>
				<view @click="selecAttrTap" class="list">
					<view class="list-item list-between">
						<text class="fs-28 color-text">အရေအတွက်ရွေးချယ်ရေး</text>
						<view class="iconfont icon-jiantou arrow-atr"></view>
					</view>
				</view>
			</view>
			<view v-if="mode!='point'" class="attribute flex-main-between">
				<text class="fs-28 color-text">ကူပွန်</text>
				<text @click="toCoupon()" class="fs-28 style-receive color-white">လက်ခံရရှိသည်</text>
			</view>
			...
			<!-- <view class="user-comment bg-white top-20">
				<view class="list">
					<view class="list-item list-between" @click="goEvaluateList(id)">
						<text class="fs-28 color-text">ထုတ်ကုန်ပြန်လည်သုံးသပ်（{{replyCount||0}}）</text>
				...
				<view v-for="(item,i) in reply" :key="i" class="comment-box flex-main-between color-text line-top">
					...
					<view class="comment-info flex-1 left-15">
						...
						<view v-show="item.merchantReplyContent" class="comment-reply fs-24 top-20">
							<text>ဆိုင်ရှင်ကပြန်ဖြေတယ်：{{item.merchantReplyContent}}</text>
						</view>
					</view>
				</view>
			</view> -->
			...
			<!-- <view class="bg-white top-20 comment-box">
				<view class="flex-main-between">
					...
					<view>
						<text @click="goShopManage()" class="fs-28 style-receive color-white">ဆိုင်ထဲဝင်ပါ</text>
					</view>
				</view>
				...
				<view>
					...
					<view v-else class="bnt acea-row">
						<view v-if="mode=='point'" style="background-color:#0972B9" class="btn-left" @click="joinCart">
							<text>စျေးဝယ်ခြင်းလှည်းထဲသို့ထည့်ပါ</text>
						</view>
	...
	export default {
		...
		data: function() {
			return {
				...
				isOpen: false, //အိမ်ခြံမြေအစိတ်အပိုင်းကိုဖွင့်ဖို့ရှိမရှိ
				productValue: [],
				id: 0,
				uid: 0,
				...
				shopInfo: {},
				couponList: [],
				attrTxt: "ကျေးဇူးပြု။ ရွေးချယ်ပါ",
				attrValue: "",
				cart_num: 1, //ဝယ်ယူအရေအတွက်
				replyCount: "",
				replyChance: "",
				reply: [],
				priceName: 0,
				...
				tempName: "အမျိုးသားအခမဲ့ပို့ဆောင်ပေးခြင်း",
				mode: '',
			};
		},
		...
		onLoad(options) {
			// console.log(options)
			if (options.scene) { // စကင်ကုဒ်အခြေအနေတွေကိုင်တွယ်
		...
		mounted: function() {
			...
			this.productCon();
			// ၎င်းသည်ကုန်ပစ္စည်းအမှတ်အသားမဟုတ်ပါကလည်ပတ်မှုမှတ်တမ်းကိုထည့်ပါ
			if (this.mode != 'point') {
				addHistroy({
					id: this.id
				}).catch(console.error)
			...
			// if (this._route.query.uid && this._route.query.uid != this.uid) { // သာလွန်သည်id,ကိုယ့်ကိုယ်ကိုယ်မ，စည်းနှောင်သာလွန်စတင်
			// 	bindSuperior({
			// 		spread: this._route.query.uid
			// 	}).then(() => console.log('အောင်မြင်စွာသာလွန်မှခညျြနှောငျ！')).catch(err => {
			// 		console.log('သာလွန်ချည်နှောင်ရန်မအောင်မြင်：', err)
			// 	})
			// }
		},
		watch: {
		...
		methods: {
			...
			bindShare(){
				...
				this.wxReady.then(res=>{
					if(res){
						wx.updateAppMessageShareData({ // သူငယ်ချင်းများကိုမျှဝေပါ
						    title: '', // ခေါင်းစဉ်မျှဝေပါ
						    desc: '', // ဖော်ပြချက်မျှဝေပါ
						    link: '', // လင့်ကိုမျှဝေပါ，link domain name သို့မဟုတ် path သည်လက်ရှိစာမျက်နှာ၏တရားဝင်အကောင့်နှင့်ကိုက်ညီရမည်JSလုံခြုံရေးဒိုမိန်းအမည်မှာတသမတ်တည်းဖြစ်သည်
						    imgUrl: '', // ဝေစုအိုင်ကွန်
						    success: function () {
						      // အောင်မြင်စွာသတ်မှတ်ပါ
						    }
						  })
						wx.updateTimelineShareData({ // ခဏမျှဝေပါqqအာကာသ
						    title: '', // ခေါင်းစဉ်မျှဝေပါ
						    link: '', // လင့်ကိုမျှဝေပါ，link domain name သို့မဟုတ် path သည်လက်ရှိစာမျက်နှာ၏တရားဝင်အကောင့်နှင့်ကိုက်ညီရမည်JSလုံခြုံရေးဒိုမိန်းအမည်မှာတသမတ်တည်းဖြစ်သည်
						    imgUrl: '', // ဝေစုအိုင်ကွန်
						    success: function () {
			...
			productCon: function() {
				...
				getProductDetail(that.id, from)
					.then(res => {
						...
						// ပေး attr တာဝန်，တောင်းဆိုထားသောသတ်မှတ်ချက်များကို assign attr
			...
			// စတိုးဆိုင်သတင်းအချက်အလက်ကိုရယူပါ
			getStoreInfo: function() {
				getStoreInfo(this.storeInfo.merId).then(res => {
					this.shopInfo = res.data;
				})
			},
			//ပုံမှန်အားဖြင့်ရွေးချယ်ထား Properties ကို；
			DefaultSelect: function() {
				...
				//sort();function ကိုစီပါ:ဒီဂျစ်တယ်-အင်္ဂလိပ်-တရုတ်ဇာတ်ကောင်；
				...
				if (productSelect && productAttr.length) {
					...
					this.$set(this, "attrTxt", "ရွေးချယ်ခဲ့သည်");
					...
					this.$set(this, "attrTxt", "ကျေးဇူးပြု။ ရွေးချယ်ပါ");
				} else if (!productSelect && !productAttr.length) {
					this.$set(
						this.attr.productSelect,
						"store_name",
			...
			ChangeCartNum: function(changeValue) {
				...
				//လက်ရှိပြောင်းလဲမှု attribute တွေရယူပါ
				let productSelect = this.productValue[this.attrValue];
				//အဘယ်သူမျှမ attribute ကိုရှိလျှင်,ထုတ်ကုန်ကို default စာရင်းတန်ဖိုးကိုသတ်မှတ်
				if (productSelect === undefined && !this.attr.productAttr.length) {
					productSelect = this.attr.productSelect;
				}
				//အဘယ်သူမျှမ attribute ကိုတန်ဖိုးစာရင်းဖြစ်ပါတယ်ဆိုလိုသည်0；အတိုးနှင့်အနှုတ်လည်းမရှိပါ；
			...
			changeattr: function(msg) {
				// ပြုပြင်သတ်မှတ်ချက်များ
				// console.log(msg);
				this.attr.cartAttr = msg;
				this.isOpen = false;
			},
			//attribute ကိုရွေးချယ်ပါ；
			...
			//  Add to cart ခလုတ်ကိုနှိပ်ပါ
			joinCart: function() {
				if (this.mode == 'point') {
					uni.showToast({
						icon: 'none',
						title: 'အမှတ်ထုတ်ကုန်များကိုထပ်ထည့်။ မရပါ'
					})
					return
				}
				//0=စျေးဝယ်ခြင်းလှည်းထဲသို့ထည့်ပါ
				this.goCat(0);
			},
			// စျေးဝယ်ခြင်းလှည်းထဲသို့ထည့်ပါ；
			goCat: function(news) {
				let that = this,
					productSelect = that.productValue[this.attrValue];
				//ပွင့်လင်းဂုဏ်သတ္တိများ
				if (that.attrValue) {
					//ပုံမှန်အားဖြင့်ရွေးချယ်ထား Properties ကို，သို့သော်၊ ပိုင်ဆိုင်မှုပေါ်ပေါက်သည့် ၀ င်းဒိုးကိုမဖွင့်နိုင်ပါ၊ သို့မဟုတ်ပုံမှန်အားဖြင့်ရွေးချယ်ထားသည့်ဂုဏ်သတ္တိများကိုအသုံးပြုသူများကိုကြည့်ရှုခွင့်ပေးရန်အလိုအလျောက်ဖွင့်လှစ်သည်
					that.attr.cartAttr = !that.isOpen ? true : false;
				} else {
					if (that.isOpen) that.attr.cartAttr = true;
					else that.attr.cartAttr = !that.attr.cartAttr;
				}
				//သင် attribute popup ကိုပိတ်လိုက်မှသာလျှင်လှည်းထဲသို့ထည့်ပါ
				if (that.attr.cartAttr === true && that.isOpen === false)
					return (that.isOpen = true);
				//attribute တွေရှိပါတယ်လျှင်,ရွေးချယ်စရာမရှိ,ရွေးချယ်ရန်အသုံးပြုသူကိုပြောပါ
				if (
					...
					uni.showToast({
						title: "ကုန်ပစ္စည်းစာရင်းမလုံလောက်ပါ，ကျေးဇူးပြုပြီးအခြားရွေးပါ",
						icon: "none",
						duration: 2000
					});
					return;
				...
				postCartAdd(q)
					.then(function(res) {
						...
						if (news) {
							...
							uni.showToast({
								title: "အောင်မြင်စွာလှည်းထဲသို့ထည့်ပါ",
								icon: "success",
								duration: 2000,
								complete: () => {
			...
			//စျေး ၀ ယ်စာရင်းအရေအတွက်ကိုရယူပါ
			getCartCount: function(isAnima) {
				...
				if (isLogin) {
					getCartCount({
						...
						//စျေးဝယ်ခြင်းလှည်းထဲသို့ထည့်ပြီးနောက် attribute တွေ Reset
						if (isAnima) {
							that.animated = true;
							setTimeout(function() {
								that.animated = false;
			...
			// ဖောက်သည်ဝန်ဆောင်မှုကိုဆက်သွယ်ပါ
			contactCustomer() {
				let H5Sign,wechatSign;
				this.shopInfo.list.forEach(function (item) {
				    if (item.channelName == "Mini Program Plugin") {
						wechatSign = item.webSign;
				    }
					if (item.channelName == "ဝဘ်ဆိုက်-h5") {
						H5Sign = item.webSign;
					}
				    console.log(item);
				});
