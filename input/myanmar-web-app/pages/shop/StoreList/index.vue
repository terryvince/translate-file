      ...
      >
        ...
        <view class="row-right">
          ...
          <view class="store-distance" @click="showMaoLocation(item)">
            <text class="addressTxt" v-if="item.distance">အကွာအဝေး{{ item.distance }}ကီလိုမီတာ</text>
            <text class="addressTxt" v-else>မြေပုံကိုစစ်ဆေးပါ</text>
...
export default {
  ...
  methods: {
    ...
    // စတိုးစာရင်းအချက်အလက်များကိုရယူပါ
    getList: function() {
      if (this.loading || this.loaded) return;
      this.loading = true;
      let data = {
        latitude: this.location.latitude, //လတ္တီတွဒ်
        longitude: this.location.longitude, //လောင်ဂျီတွဒ်
        page: this.page,
        limit: this.limit
      };
      storeListApi(data)
