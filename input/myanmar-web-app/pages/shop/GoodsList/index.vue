	...
	<view class="productList" ref="container">
      ...
      <view class="search bg-gray acea-row row-between-wrapper">
       <view class="input acea-row row-between-wrapper flex-1">
          ...
          <input placeholder="ကျေးဇူးပြု၍ ရှာဖွေရန်သော့ချက်စာလုံးကိုရိုက်ထည့်ပါ" v-model="" />
        </view>
        <view
          class="iconfont"
		...
		<view class="nav acea-row row-middle">
			...
			<view :class="['item','relative']" @click="set_where(0)">{{ title ? title : "ခွဲခြား" }}</view>
			 ...
			 :key="item.id" :title="item.storeName">
				...
				<view class="text" :class="Switch === true ? '' : 'on'">
					...
					<view class="vip acea-row row-between-wrapper" :class="Switch === true ? '' : 'on'">
						<view class="fs-20 color-text-secondary txt-bold">{{item.shopName||'ဆိုင်နာမည်အမည်မရှိပါ'}}</view>
						<view>ရောင်းချခဲ့သည်{{ item.sales }}အပိုင်းအစများ</view>
					</view>
				</view>
			</view>
		</view>
	...
	export default {
		...
		methods: {
			...
			get_product_list() {
				...
				let q = that.where;
				// type 0 အထွေထွေကုန်ပစ္စည်း 1ကုန်ပစ္စည်းအမှတ် 2 Featured ထုတ်ကုန်များ
				if (type) {
					q.type = type
				}
				getProducts(q).then(res => {
					...
					that.loadend = res.data.length < that.where.limit; //ဒေတာအားလုံး loaded ရှိမရှိဆုံးဖြတ်ပါ；
			...
			//ဖြစ်ရပ်ကိုင်တွယ်ခြင်းကိုနှိပ်ပါ
