<template>
  <view class="statistical-page" ref="container">
    <view class="navs">
      <view class="list">
        <view class="item" :class="time == 'today' ? 'on' : ''" @click="setTime('today')">ဒီနေ့</view>
        <view class="item" :class="time == 'yesterday' ? 'on' : ''" @click="setTime('yesterday')">မနေ့က</view>
        <view class="item" :class="time == 'seven' ? 'on' : ''" @click="setTime('seven')">မကြာသေးမီက7နေ့</view>
        <view class="item" :class="time == 'month' ? 'on' : ''" @click="setTime('month')">ဤလတွင်</view>
      </view>
    </view>
    <view class="wrapper">
      <view class="title">{{ title }}{{ this.where.type == 1 ? "လည်ပတ်ငွေကြေး（ယွမ်）" : "အမှာစာအသံအတိုးအကျယ်（မျှဝေပါ）" }}</view>
      <view class="money">{{ time_price }}</view>
    </view>
    <!-- <view class="chart">
      <view class="company">{{ where.type === 1 ? "ယူနစ်（ယွမ်）" : "ယူနစ်（မျှဝေပါ）" }}</view>
    ...
    <view class="public-wrapper">
      ...
      <view class="nav acea-row row-between-wrapper">
        ...
        <view class="browse">အမှာစာအသံအတိုးအကျယ်</view>
...
export default {
  ...
  data: function() {
    return {
      polar: {
        ...
        xAxis: [
          {
            type: "category",
            boundaryGap: false,
            data: ["တနင်္လာနေ့မှာ", "အင်္ဂါနေ့မှာ", "ဗုဒ္ဓဟူးနေ့မှာ", "ကြာသပတေးနေ့", "သောကြာနေ့", "စနေနေ့မှာ", "တနင်္ဂနွေနေ့"],
            splitLine: {
              show: false
            },
            axisLine: {
              lineStyle: {
                color: "#999",
                width: 1 //ဤတွင်ကိုမီးမောင်းထိုးပြထည့်သွင်းထားသည်
              }
            }
          }
        ],
        ...
        series: [
          {
            name: "အီးမေးလ်စျေးကွက်",
            type: "line",
            stack: "စုစုပေါင်း",
            itemStyle: {
              normal: {
                color: "#2291f8", //Breakpoint အရောင်
                lineStyle: {
                  color: "#2291f8" //Polyline အရောင်
                }
              }
            },
      ...
      ismulti: false,
      monFirst: true,
      clean: false, //အကျဉ်းချုပ် mode ကို
      lunar: false, //လပြက္ခဒိန်ကိုပြပါ
      renderValues: [],
      monthRange: [],
      current: false,
      where: {
      ...
      types: "", //ပုံစံ|order=အမိန့်အရေအတွက်|price=လည်ပတ်ငွေကြေး
      time: "", //အချိန်|today=ဒီနေ့|yesterday=မနေ့က|month=ဤလတွင်
      title: "", //အချိန်|today=ဒီနေ့|yesterday=မနေ့က|month=ဤလတွင်
      growth_rate: "", //ကြီးထွားနှုန်း
      increase_time: "", //ကြီးထွားနှုန်း
      increase_time_status: "", //ကြီးထွားနှုန်း
      time_price: "", //ကြီးထွားနှုန်း
      loaded: false,
      loading: false,
      filter: {
        page: 1,
  ...
  computed: {
    monthRangeText() {
      return this.monthRange.length ? "သတ်မှတ်ထားတဲ့" : "သတ်မှတ်ထားသောအကွာအဝေး";
    }
  },
  methods: {
    getIndex: function() {
    ...
    switchMode() {
      ...
      setTimeout(() => {
        // this.$refs.calendar.renderer(2019, 1); //ပြန်ကြားရေး2018နှစ်8လ
      }, 0);
    },
    ...
    renderer() {
      ...
      // this.$refs.calendar.renderer(2018, 8); //ပြန်ကြားရေး2018နှစ်8လ
    },
    select(val, val2) {
      if (this.isrange) {
