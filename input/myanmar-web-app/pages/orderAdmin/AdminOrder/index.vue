<template>
  <view class="order-details pos-order-details">
    <view class="header acea-row row-middle">
      ...
      <view class="data">
        <view class="order-num">အမှာစာ：{{ orderInfo.orderId }}</view>
        <view>
          <text class="time">
    ...
    <view class="public-total">
      စုစုပေါင်း{{ orderInfo.totalNum }}ပစ္စည်းများ，ပေးဆောင်
    ...
    <view class="wrapper">
      ...
      <view class="item acea-row row-between">
        ...
        <view class="conter">{{ orderInfo.paid == 1 ? "ပေးဆောင်သည်" : "မရတဲ့" }}</view>
      </view>
      <view class="item acea-row row-between">
    ...
    >
      <view class="item acea-row row-between">
        ...
        <view class="conter" v-if="orderInfo.delivery_type === 'express'">အမြန်ပေးပို့ခြင်း</view>
        <view class="conter" v-if="orderInfo.delivery_type === 'send'">ပို့ခြင်း</view>
      </view>
      <view class="item acea-row row-between">
        <view v-if="orderInfo.delivery_type === 'express'">Courier ကုမ္ပဏီ：</view>
        <view v-if="orderInfo.delivery_type === 'send'">Delivery လူ：</view>
        <view class="conter">{{ orderInfo.delivery_name }}</view>
      </view>
      <view class="item acea-row row-between">
        <view v-if="orderInfo.delivery_type === 'express'">ခြေရာကောက်နိုင်မည့်နံပါတ်：</view>
        <view v-if="orderInfo.delivery_type === 'send'">လူကိုဖုန်းပို့ပါ：</view>
        <view class="conter">
    ...
    <view class="footer acea-row row-right row-middle">
      ...
      <view class="bnt cancel" @click="modify(0)" v-if="types == 0">တစ်ချက်နှိပ်လျှင်စျေးနှုန်းပြောင်းလဲခြင်း</view>
      <view class="bnt cancel" @click="modify(0)" v-if="types == -1">ယခုပြန်အမ်းငွေ</view>
      <view
        class="bnt cancel"
        v-if="orderInfo.pay_type === 'offline' && orderInfo.paid === 0"
        @click="offlinePay"
      >ငွေပေးချေမှုကိုအတည်ပြုပါ</view>
      <view class="bnt delivery" v-if="types == 1" @click="goGoodsDeliver(orderInfo)">တင်ပို့ရန်</view>
    </view>
    <PriceChange
      :change="change"
      :orderInfo="orderInfo"
