			...
			<view class="item" v-for="(order,orderListIndex) in orderList" :key="orderListIndex">
				<view class="title acea-row row-between-wrapper">
					<view class="acea-row row-middle">
						...
						<block v-if="order.pinkName.indexOf('အရေးပါသော') != -1">
							<image src="https://res.sdebs.com/icon-order-ji.png" class="classIcon">
								<view class="text">အရေးပါသော</view>
							</image>
						</block>

						<block v-if="order.pinkName.indexOf('အုပ်စုတစ်ခုနှင့်ဆက်သွယ်ပါ') != -1">
							<image src="https://res.sdebs.com/icon-order-ping.png" class="classIcon">
								<view class="text">အုပ်စုတစ်ခုနှင့်ဆက်သွယ်ပါ</view>
							</image>
						</block>

						<block v-if="order.pinkName.indexOf('ဆူး') != -1">
							<image src="https://res.sdebs.com/icon-order-miao.png" class="classIcon">
								<view class="text">ဆူး</view>
							</image>
						</block>

						<block v-if="order.pinkName.indexOf('သာမန်') != -1">
							<!-- <image src="https://res.sdebs.com/icon-order-miao.png" class="classIcon"> -->
							<view class="text">{{order.useIntegral > 0 ? 'Points မှာယူသည်' : 'ပုံမှန်အမိန့်'}}</view>
							<!-- </image> -->
						</block>
						<!-- {{ order.createTime }} -->
					</view>
				...
				<view class="totalPrice flex-main-end" style="padding-bottom: 0rpx !important;">
					<view class="flex-cross-center top-30">
						စုစုပေါင်း{{ order.cartInfo.length || 0 }}ပစ္စည်းများ，ကိုင်တွယ်:
						<text class="money font-color-red">￥{{ order.payPrice }}</text>
					</view>
				</view>
				<view class="bottom acea-row row-right row-middle">
					<template v-if="order._status._type == 0">
						<view class="bnt cancelBnt" @click="cancelOrder(order)">အော်ဒါဖျက်သိမ်းသည်</view>
						<view class="bnt bg-color-green" @click="goOrderDetails(order)">ချက်ချင်းငွေပေးချေခြင်း</view>
					</template>
					<!-- အမှာစာအသေးစိတ်ကိုထုတ်ကုန်များရှိရမည်ဟုယူဆ -->
					<template v-if="(order._status._type == 1 || order._status._type == 2 ) && order.cartInfo[0].productInfo.id>6">
						<view class="bnt cancelBnt" @click="goGoodsReturn(order)">ပြန်အမ်းငွေတောင်းခံပါ</view>
					</template>
					<template v-if="order._status._type == 1 || order._status._type == 9">
						<view class="bnt bg-color-green" @click="goOrderDetails(order)">အသေးစိတ်ကြည့်ရှုပါ</view>
					</template>
					<template v-if="order._status._type == 2">
						<view class="bnt default" @click="goLogistics(order)">ထောက်ပံ့ပို့ဆောင်ရေးကိုကြည့်ပါ</view>
						<view class="bnt bg-color-green" @click="takeOrder(order)">လက်ခံရရှိကိုအတည်ပြုပါ</view>
					</template>
					<template v-if="order._status._type == 3">
						...
						<!--&gt;-->
						<!--ထောက်ပံ့ပို့ဆောင်ရေးကိုကြည့်ပါ-->
						<!--</view>-->
						<view class="bnt bg-color-green" @click="goOrderDetails(order)">အကဲဖြတ်ရန်</view>
					</template>
					<template v-if="order._status._type === 4">
						<view class="bnt bg-color-green" @click="goOrderDetails(order)">အမိန့်စစ်ဆေးသည်</view>
					</template>
				</view>
			</view>
		</view>
		...
		methods: {
			...
			goOrderDetails(order) {
				this.$yrouter.push({
					...
					query: {
						id: order._status._type ==0 ? order.orderId : order.extendOrderId  // ငွေပေးချေမှုကိုဆိုင်းငံ့ထားပါid
					}
				});
			},
			dataFormat,
