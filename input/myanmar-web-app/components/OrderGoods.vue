...
<template>
	<view class="orderGoods">
		<view class="total">စုစုပေါင်း{{ cartInfo.length }}ပစ္စည်းများ</view>
			...
			<view class="item acea-row row-between-wrapper" v-for="cart in cartInfo" :key="cart.id">
				...
				<view class="text">
					...
					<view class="money font-color-red">
						...
						<text>{{ cart.truePrice }}{{type==1 ? 'အရေးပါသော':''}}</text>
					</view>
					<view class="evaluate" v-if="evaluate == 3" @click="routerGo(cart)">အကဲဖြတ်ခြင်း</view>
				</view>
			</view>
		</view>
	</view>
