<template>
  <view>
    <view class="switchWindow" :class="switchActive === true ? 'on' : ''">
      ...
      <!-- Mini Program အကောင့်သို့ပြောင်းရန်ရွေးချယ်မလား？ -->
      <view class="info">
        <text>ပြောင်းချင်လား</text>
        <text class="font-color" v-if="login_type === 'h5'">WeChat အကောင့်</text>
        <text class="font-color" v-else>လက်ကိုင်ဖုန်းသုံးစွဲသူများ</text>
        <text>？</text>
      </view>
...
export default {
  ...
  methods: {
    switchClose: function() {
      this.$emit("changeswitch", false); //$emit():မှတ်ပုံတင်ခြင်းကိစ္စ；
    },
    switchH5() {
      let that = this;
      uni.showLoading({
